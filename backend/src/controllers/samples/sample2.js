...
exports.createTerm = async (req, res) => {
    try {
        ...
        const exists = await Term.findOne({ name, year });
        if (exists) return res.status(409).json({ message: 'Term already exists for this year' });

        const term = await Term.create({ name, year });
...
exports.updateTerm = async (req, res) => {
    try {
        const { id } = req.params;
        const updated = await Term.findByIdAndUpdate(id, req.body, { new: true });
        res.json(updated);
    } catch (err) {
        res.status(500).json({ message: 'Server error' });
    }
