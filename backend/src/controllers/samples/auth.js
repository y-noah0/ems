...
const register = async (req, res) => {
    try {
        ...
        const { email, password, fullName, role, registrationNumber, schoolId, phoneNumber, subjects, profilePicture } = req.body;
...
const requestPasswordReset = async (req, res) => {
    try {
        ...
        req.io.to(user.id).emit('notification', { message: 'Password reset link sent' });
...
const resetPassword = async (req, res) => {
    try {
        ...
        const user = await User.findOne({ emailVerificationToken: token });
        if (!user) {
            logger.warn('Invalid reset token', { token, ip: req.ip });
            return res.status(400).json({ success: false, message: 'Invalid or expired token' });
        }
        ...
        user.emailVerificationToken = null;
        user.emailVerified = true;
        await user.save(); // tokenVersion incremented in pre('save')

        req.io.to(user.id).emit('notification', { message: 'Password reset successfully' });
...
const enable2FA = async (req, res) => {
    try {
        ...
        if (!user) {
            logger.warn('User not found for 2FA', { userId: req.user.id, ip: req.ip });
            return res.status(404).json({ success: false, message: 'User not found' });
        }

        const secret = speakeasy.generateSecret({ name: `EduApp:${user.email || user.registrationNumber}` });
        user.twoFactorSecret = secret.base32;
        user.twoFactorEnabled = true;
        await user.save();

        req.io.to(user.id).emit('notification', { message: '2FA enabled successfully' });
...
const updateProfile = async (req, res) => {
    try {
        ...
        const { email, phoneNumber, profilePicture, preferences } = req.body;
        const user = await User.findById(req.user.id);

        if (email && email !== user.email) {
            const existingUser = await User.findOne({ email });
            if (existingUser) {
                logger.warn('Email already in use', { email, userId: req.user.id, ip: req.ip });
            ...
            await transporter.sendMail({
                ...
                html: `<p>Please verify your new email by clicking <a href="${process.env.APP_URL}/verify-email?token=${verificationToken}">here</a>.</p>`
            });
            req.io.to(user.id).emit('notification', { message: 'Verification email sent for new email' });
        }
