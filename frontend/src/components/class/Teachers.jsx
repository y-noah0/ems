...
const TeacherManagement = () => {
    ...
    useEffect(() => {
        if (!currentUser) {
            setNotification({
                type: "error",
                message: "Please log in to view teachers.",
            });
            return;
        }
        if (currentUser.school && !loading) {
                ...
                setTeachersData(
                    teachersData.map((t) =>
                        t._id === selectedTeacher._id
                            ? { ...payload, _id: selectedTeacher._id }
                            : t
                    )
                );
                setNotification({
    ...
    const handleDelete = async (teacherId) => {
        if (window.confirm("Are you sure you want to delete this teacher?")) {
            try {
                await teacherService.api.delete(`/teachers`, {
                    data: { schoolId: currentUser.school, teacherId },
                });
                setTeachersData(
                    teachersData.filter((t) => t._id !== teacherId)
                );
                setNotification({
                    type: "success",
    ...
    return (
        <div className="min-h-screen bg-gray-100 p-4 sm:p-6 lg:p-8">
            <div className="max-w-7xl mx-auto">
                ...
                {notification && (
                    <div
                        className={`fixed top-4 right-4 px-4 py-2 rounded-lg shadow-lg text-white animate-slide-in ${
