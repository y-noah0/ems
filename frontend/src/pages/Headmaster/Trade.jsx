...
export default function Trade() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setTrade(tradeData);
                const subjectsList = await subjectService.getSubjects(schoolId);
                setAllSubjects(subjectsList);
                // Assigned subjects
                const assigned = subjectsList.filter(
                    (s) =>
                        Array.isArray(s.trades) &&
                        s.trades
                            .map((t) => t._id?.toString())
                            .includes(tradeData._id)
                );
                setTradeSubjects(assigned);
                // Available subjects
                const available = subjectsList.filter(
                    (s) =>
                        !Array.isArray(s.trades) ||
                        !s.trades
                            .map((t) => t._id?.toString())
        ...
        fetchData();
    }, [id, navigate, showToast, schoolId]);
    ...
    const handleAddSubject = async () => {
        ...
        try {
            ...
            // build new trades array for subject
            const currentTrades =
                subject.trades?.map((t) => t._id?.toString()) || [];
            const updatedTrades = Array.from(
                new Set([...currentTrades, trade._id])
            );
            await subjectService.updateSubject(subject._id, {
                schoolId,
                trades: updatedTrades,
            });
            // update local state
            setTradeSubjects((prev) => [...prev, subject]);
    ...
    const handleRemoveSubject = async (subjectId) => {
        try {
            ...
            const currentTrades =
                subject.trades?.map((t) => t._id?.toString()) || [];
            const updatedTrades = currentTrades.filter(
                (tId) => tId !== trade._id
            );
            await subjectService.updateSubject(subject._id, {
                schoolId,
            ...
            setTradeSubjects((prev) => prev.filter((s) => s._id !== subjectId));
            setAvailableSubjects((prev) => [...prev, subject]);
            showToast(`${subject.name} removed from ${trade.name}`, "success");
        } catch (err) {
            showToast(`Failed to remove subject: ${err.message}`, "error");
        }
    };
                ...
                <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex gap-4">
                        <Button
                            ...
                            size="xs"
                            onClick={() =>
                                navigate("/headmaster/trades-offered")
                            }
