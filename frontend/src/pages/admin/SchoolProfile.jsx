    ...
    const fetchStatistics = async (schoolId) => {
        try {
            ...
            try {
                ...
                const schoolTeachers = allStaff.filter(staff => 
            ...
            try {
                ...
                studentCount = classes.reduce((total, classItem) => {
                    return total + (classItem.students ? classItem.students.length : 0);
                }, 0);
            } catch (classErr) {
    ...
    if (!school) {
        return (
            <Layout>
                <div className="px-6 py-4">
                    <div className="flex items-center justify-center h-64">
                        <div className="text-lg text-gray-600">School not found</div>
                    </div>
                </div>
            </Layout>
        );
                ...
                <div className="max-w-4xl mx-auto">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                            ...
                            <div className="space-y-3">
                                <div>
                                    ...
                                    <p className="text-sm text-gray-900">{school.contactPhone || 'Not provided'}</p>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-500">Email</label>
                                    <p className="text-sm text-gray-900">{school.contactEmail || 'Not provided'}</p>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-500">Address</label>
                                    <p className="text-sm text-gray-900">{school.address || 'Not provided'}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                            ...
                            <div className="space-y-3">
                                <div>
                                    <label className="block text-sm font-medium text-gray-500">Headmaster</label>
                                    <p className="text-sm text-gray-900">{school.headmaster || 'Not assigned'}</p>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-500">System Type</label>
                                    <p className="text-sm text-gray-900">{school.category}</p>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-500">Trades Offered</label>
                                    <div className="flex flex-wrap gap-1 mt-1">
                                        {school.tradesOffered && school.tradesOffered.length > 0 ? (
                                            school.tradesOffered.map((trade, index) => (
                                                <span 
                                                    key={index}
                                            ...
                                            <span className="text-sm text-gray-500">No trades assigned</span>
                                        )}
                                    </div>
                                </div>
                            </div>
